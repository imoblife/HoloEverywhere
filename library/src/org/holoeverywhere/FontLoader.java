...
public class FontLoader {
	...
	static {
		...
		ROBOTO_REGULAR = new RobotoRawFont(R.raw.roboto_regular)
				.setFontStyle(TEXT_STYLE_NORMAL);
		ROBOTO_BOLD = ROBOTO_REGULAR;
		ROBOTO_ITALIC = ROBOTO_REGULAR;
		ROBOTO_BOLDITALIC = ROBOTO_REGULAR;
		ROBOTO_CONDENSED = ROBOTO_REGULAR;
		...
		ROBOTO_BOLDCONDENSEDITALIC = new RobotoRawLazyFont(
				"roboto_boldcondenseditalic").setFontStyle(TEXT_STYLE_BOLD
				| TEXT_STYLE_CONDENDSED | TEXT_STYLE_ITALIC);
	...
	private static void applyInternal(View view, Font font) {
		...
		if (view instanceof FontStyleProvider) {
			...
			if (view.getTag(R.id.fontLoaderFont) == font
					...
					&& equals(view.getTag(R.id.fontLoaderFontFamily),
							fontFamily)) {
				return;
			}
	...
	public static Object[] parseFontStyle(String string) {
		...
		return new Object[] {
				textStyle,
				fontFamily == null && textStyle == TEXT_STYLE_NORMAL ? string
						: fontFamily };
	}
	...
	public static class Font implements Cloneable {
		...
		public boolean available(Context context, String fontFamily,
				int fontStyle) {
			return mFontFamily == null ? fontFamily == null : mFontFamily
					.equals(fontFamily) && mFontStyle == fontStyle;
		}
	...
	public static class FontCollector extends Font {
		...
		public boolean available(Context context, String fontFamily,
			...
			return font != null
					&& font.available(context, fontFamily, fontStyle);
		}
		...
		protected Typeface getTypeface(String fontFamily, int fontStyle) {
			final Font font = findFont(fontFamily, fontStyle);
			return font != null ? getTypeface(font, fontFamily, fontStyle)
					: null;
		}
		...
		public FontCollector unregister(String fontFamily, int fontStyle) {
			for (int i = 0; i < mFonts.size(); i++) {
				...
				if (FontLoader.equals(fontFamily, font.mFontFamily)
						&& font.mFontStyle == fontStyle) {
					mFonts.remove(font);
					return this;
				}
	...
	public static class RawFont extends Font {
		...
		protected Typeface loadRawTypeface() {
			final File fontFile = new File(getContext().getCacheDir(),
					"font_0x" + Integer.toHexString(mRawResourceId));
			if (fontFile.exists()) {
				if (sApplicationInstallDate == -1) {
					try {
						final Context context = getContext();
						final PackageInfo ai = context.getPackageManager()
								.getPackageInfo(context.getPackageName(), 0);
						if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
							sApplicationInstallDate = Math.max(
									ai.lastUpdateTime, ai.firstInstallTime);
						} else {
							sApplicationInstallDate = new File(
									ai.applicationInfo.sourceDir)
									.lastModified();
						}
	...
	public static class RawLazyFont extends RawFont {
		...
		private void setRawResourceId(Context context) {
			final int id = context.getResources().getIdentifier(
					mRawResourceName, "raw", context.getPackageName());
			if (id == 0) {
				throw new IllegalStateException(
						"Could not find font in raw resources: "
								+ mRawResourceName);
			}
			setRawResourceId(id);
		}
